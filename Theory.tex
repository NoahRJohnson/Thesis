...
Using equations \ref{eqTotalProb} and \refeq{eqCondProb}, one can derive a formula known as Bayes' Rule. 
...
\end{equation}
In the future this will prove to be a useful tool to update a PDF with new information. When presented with new information that  \(Y = y\), one may use Bayes' Rule to transform the current PDF \(p(x)\) into the new posterior PDF \(p(x \mathbin{\vert} y)\). %from the inverse conditional probability \(p(y \mathbin{\vert} x)\) and the prior PDF \(p(x)\).
...
Now we can compute the minimum:
\begin{multline}
...
\(bel(x_t)\), as all PDFs, must be normally distributed. We will denote its mean and covariance matrix by \(\mu\) and \(\Sigma\). This time there is no integral, and so we can immediately apply Lemma \ref{lemmaHessianInverse}. 
